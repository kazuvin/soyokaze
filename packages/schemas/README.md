...
OpenAPI定義ファイル（`openapi.yaml`や`components/`, `paths/`）を編集した後、以下の手順で更新してください：

1. `npm run generate` でスキーマとクライアントを再生成
2. サーバーを再起動
...
3. `server.js` にモックの実装を追加  
4. `npm run generate` でスキーマとクライアントを再生成
5. サーバーを再起動
